...
export async function sendFirebaseReleaseEmail(
  adminEmails: string[],
  appName: string,
  version: string,
  buildNumber: string,
  ...
  const dashboardUrl = process.env.NEXT_PUBLIC_APP_URL || baseUrl;
  ...
  const html = `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f8fafc; border-radius: 16px; overflow: hidden; border: 1px solid #ffca28;">
      ...
      <div style="padding: 40px 32px; background-color: #ffffff; text-align: center;">
        ...
        <div style="margin-top: 40px;">
          <a href="${dashboardUrl}" style="background-color: #ffa000; color: white; padding: 14px 32px; text-decoration: none; border-radius: 12px; font-weight: bold; font-size: 15px; display: inline-block; box-shadow: 0 4px 10px rgba(255, 160, 0, 0.2);">Ver Dashboard Firebase</a>
        </div>
      </div>
